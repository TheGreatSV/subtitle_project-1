  ...
  return (
    <div className="Desktop">
      <header className="header">
        ...
        <button className='Desktopbuttonvid' onClick={selectVideo}>Open a Video</button>
        <input type="file" hidden ref={fileInputRef} accept="video/*" onChange={handleFileChange} />
        </div>
      </header>

      <div className="main-contentDesktop">
      <div className="scroll-containerDesktop">
      <div className="row">
     
      <div className="video-sectionDesktop">
          <ReactPlayer
           ref={playerRef}
            className="react-playerDesktop"
            url={url}
            width="100%"
            height="300px"
            controls={true}
        ...
        <div className="entries-sectionDesktop">
         
        
          {Divs.map((div, index) => (
            <div key={div} className={`entry-cardDesktop ${
              highlightIndices.includes(index) ? 'entry-card-highlightDesktop' : ''
            }`}>
              <div>{+index+1}</div>
